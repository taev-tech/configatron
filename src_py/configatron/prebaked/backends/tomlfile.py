...
class TomlFileBackend(CfgBackend):
    """A config backend that uses a toml file for storing config values.
    ...
    def load_sync(
        ...
        toml_text = file_path.read_text('utf-8')
        return self._load(toml_text, request, full_keyspace)
    ...
    async def load_async(
        ...
        toml_text = await file_path.read_text('utf-8')
        return self._load(toml_text, request, full_keyspace)
    ...
    def _load(
        ...
        toml_obj = tomllib.loads(toml_text)
        retval: dict[CfgFieldDesc, Any] = {}

        for namespace, field_tuples in full_keyspace.by_namespace.items():
            if namespace is None:
                target_toml_obj = toml_obj
            else:
                target_toml_obj = toml_obj.get(namespace, {})
